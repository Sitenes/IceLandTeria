...
(function(html) {
    ...
    * ---------------------------------------------------- */ 
    const sstabs = function(nextTab = false) {
        ...
        const tabLists = document.querySelectorAll('.tab-nav__list');

        if (!tabLists.length) return;

        tabLists.forEach(function(tabList) {

            const menuContent = tabList.closest('.s-menu__content');
            if (!menuContent) return;

            const tabPanels = menuContent.querySelectorAll('.tab-content__item');
            const tabItems = tabList.querySelectorAll('li');
            const tabLinks = [];

            if (!(tabList && tabPanels.length)) return;

            const instanceId = Array.from(tabLists).indexOf(tabList);
            ...
            tabPanels.forEach(function(panel, i) {
                ...
                let attributes = {
                    'role': 'tabpanel',
                    'aria-hidden': 'true',
                    'aria-labelledby': 'tab-link-' + instanceId + '-' + i
                };
                ...
                if (nextTab) {
                    ...
                    nextTabLink.setAttribute('href', '#tab-link-' + instanceId + '-' + nextTabLinkIndex);
